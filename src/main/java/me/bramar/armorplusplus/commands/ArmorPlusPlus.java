...
public class ArmorPlusPlus implements CommandExecutor, Listener {	
	...
	public ArmorPlusPlus(Plugin plugin, me.bramar.armorplusplus.ArmorPlusPlus main) {
		this.plugin = plugin;
		this.main = main;
	}
				...
				if(args[0].equalsIgnoreCase("gui")) {
					...
					main.uc.getVersion((version) -> {
						if(version == null) {
							...
							p.sendMessage(main.msg);
							try {
								String msg = ChatColor.YELLOW + "=====================" + ChatColor.RESET + "\n" + ChatColor.YELLOW + "Current version: " + main.getDescription().getVersion() + ChatColor.RESET + "\n" + ChatColor.YELLOW + "Latest version: " + version
										+ ChatColor.RESET + "\n" + ChatColor.YELLOW + "Spigot Plugin name: " + main.uc.getPluginName() + ChatColor.RESET + "\n" + ChatColor.YELLOW + "Plugin tag: " + main.uc.getPluginTag() + ChatColor.RESET + "\n" + ChatColor.YELLOW + "Plugin author: " + main.uc.getAuthor() + ChatColor.RESET + "\n" + ChatColor.YELLOW + "Latest version title: " + main.uc.getLatestTitle() + ChatColor.RESET + "\n" + ChatColor.YELLOW + "Latest version changelog:" + ChatColor.RESET + "\n" + ChatColor.YELLOW + main.uc.getLatestChangelog().replaceAll("\n", ChatColor.RESET + "\n" + ChatColor.YELLOW) + ChatColor.RESET + "\n\n" + ChatColor.YELLOW + "Download size: " + main.uc.getDownloadSize() + ChatColor.RESET + "\n" + ChatColor.YELLOW + "Download at: " + main.uc.getDownloadUrl() + ChatColor.RESET + "\n" + ChatColor.YELLOW + "Additional information: " + main.uc.getAdditionalInfo() + ChatColor.RESET + "\n" + ChatColor.YELLOW + "=====================";
								p.sendMessage(msg);
							}catch(Exception e) {
								p.sendMessage(ChatColor.RED + "Failed to get specific info! " + ChatColor.DARK_RED + "ERROR: " + e.getMessage());
							}
						}
					});
					...
					if(args.length == 1) {
						p.sendMessage(ChatColor.RED + "Usage: /armorplusplus give <player> <id>");
						return true;
					}
					Player player = null;
					try {
					...
					if(value == -1 || armorname == "") {
						...
						if(itemname == "" || val == -1) {
							...
							main.invevents.items(new InventoryClickEvent(view, SlotType.CONTAINER, val, ClickType.LEFT, InventoryAction.NOTHING));
							p.sendMessage(ChatColor.GREEN + "Successfully given " + player.getName() + " " + Method.getCustomItems().get(itemname) + " item!");
						}
						return true;
					}
					openGui(player);
					...
					main.ar.activeRecipes = new ArrayList<>();
					main.loadRecipe();
					...
					if(p.hasPermission("armorplusplus.checkupdate") || p.hasPermission("armorplusplus.cheat") || p.hasPermission("armorplusplus.reload") || p.hasPermission("armorplusplus.config")) p.sendMessage(ChatColor.RED + "Invalid argument. Only "
							+ "'gui', 'check', 'reload', 'give', 'resetconfig', and 'items' is allowed");
					else p.sendMessage(ChatColor.RED + "No permission!");
				}
			}
		}else {
					...
					main.uc.getVersion((version) -> {
						if(version == null) {
							...
							try {
								...
								e1.printStackTrace();
								p.sendMessage(ChatColor.RED + "Failed to get specific info! " + ChatColor.DARK_RED + "ERROR: " + e1.getMessage());
							}
						}
					});
				}else if(args[0].equalsIgnoreCase("reload")) {
	...
	public Inventory openItems(Player p) {
		Inventory gui = Bukkit.createInventory(p, 54, ChatColor.GOLD + "All Armor++'s custom items");
		
		gui.setItem(0, Method.getItem(Material.POTION, ChatColor.DARK_PURPLE + "Witch Potion"));
		try {
			p.openInventory(gui);
		}catch(Exception e) {
	...
	public Inventory openGui(Player p) {
		...
		gui.setItem(34, Method.getCommandItem(main.ar.getWitchPotion(), "WITCH"));
		try {
			p.openInventory(gui);
		}catch(Exception e) {
